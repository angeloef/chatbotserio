...
async def search_properties(
    ...
    async with async_session() as session:
        ...
        if operation:
            stmt = stmt.where(Property.operation == operation.lower())
        if tipo:
            ...
            stmt = stmt.where(Property.property_type == mapped)
        if zona:
            stmt = stmt.where(Property.zone.ilike(f"%{zona}%"))
        ...
        for p in properties:
            op_label = "Alquiler" if p.operation == "alquiler" else "Venta"
            price_str = f"${p.price:,.0f}/mes" if p.operation == "alquiler" else f"${p.price:,.0f}"
            tipo_str = p.property_type.capitalize()
            bedrooms_str = f"{p.bedrooms} dormitorio{'s' if p.bedrooms != 1 else ''}" if p.bedrooms > 0 else "Monoambiente"

            lines.append(
                f"  [{p.id}] {tipo_str} en {p.zone} — {op_label} {price_str}\n"
                f"       {bedrooms_str} | {p.covered_m2:.0f}m² | {p.title}"
            )
